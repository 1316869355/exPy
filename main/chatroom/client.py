...
class LoginFrame(wx.Frame):
    ...
    def login(self, event):
        ...
        try:
            ...
            con.open(serverAddress[0], port = int(serverAddress[1]), timeout = 500)
            response = con.read_some().decode('utf-8')
            print(response)
            if response != '200':
                self.showDialog('Error2', 'Connect Fail!', (95, 20))
                return
            con.write('login '.encode('utf-8')  + str(self.userName.GetLineText(0)).encode('utf-8') + b'\n')
            response = con.read_some().decode('utf-8')
            print(response)
            if response == 'UserName Empty':
                self.showDialog('Error', 'UserName Empty!', (135, 20))
            elif response == 'UserName Not Exist':
                self.showDialog('Error', 'UserName Exist!', (135, 20))
            else:
                self.Close()
                ChatFrame(None, -1, title = 'ShiYanLou Chat Client', size = (500, 350))
        except Exception as err:
            print ('Exception: ', err)
            self.showDialog('Error1', 'Connect Fail!', (95, 20))
            raise err
...
class ChatFrame(wx.Frame):
    ...
    def send(self, event):
        ...
        if message != '':
            con.write('say '.encode('utf-8') + message.encode('utf-8') + b'\n')
            self.message.Clear()
